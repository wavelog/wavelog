...
class Logbook extends CI_Controller {
	...
	function json($tempcallsign, $tempband, $tempmode, $tempstation_id = null, $date = "") {
		...
		$callbook = $this->logbook_model->loadCallBook($callsign, $this->optionslib->get_option('callbook_fullname'));
	...
	*/
	...
	function partial($id, $band = null) {
		...
		if (!empty($logbooks_locations_array) && $query->num_rows() > 0) {
			...
			if ($this->optionslib->get_option('callbook_provider') == "qrz" && $this->optionslib->get_option('callbook_username') != null && $this->optionslib->get_option('callbook_password') != null) {
				// Lookup using QRZ
				$this->load->library('qrz');

				if(!$this->session->userdata('qrz_session_key')) {
					$qrz_session_key = $this->qrz->session($this->optionslib->get_option('callbook_username'), $this->optionslib->get_option('callbook_password'));
					$this->session->set_userdata('qrz_session_key', $qrz_session_key);
				}
				$callsign['callsign'] = $this->qrz->search($id, $this->session->userdata('qrz_session_key'), $this->optionslib->get_option('callbook_fullname'));
			...
			} else if ($this->optionslib->get_option('callbook_provider') == "hamqth" && $this->optionslib->get_option('callbook_username') != null && $this->optionslib->get_option('callbook_password') != null) {
				// Load the HamQTH library
				$this->load->library('hamqth');

				if(!$this->session->userdata('hamqth_session_key')) {
					$hamqth_session_key = $this->hamqth->session($this->optionslib->get_option('callbook_username'), $this->optionslib->get_option('callbook_password'));
				...
				$callsign['error'] = sprintf(__("Lookup not configured or disabled. Please check your %sCallbook Settings%s."), '<a target="_blank" href="' . site_url('options/callbook') . '">', "</a>");
			}
			...
			if ($query->num_rows() > 0) {
				...
				if ($iota_search->num_rows() > 0) {
					...
					if ($this->optionslib->get_option('callbook_provider') == "qrz" && $this->optionslib->get_option('callbook_username') != null && $this->optionslib->get_option('callbook_password') != null) {
						...
						$data['callsign'] = $this->qrz->search($id, $this->session->userdata('qrz_session_key'), $this->optionslib->get_option('callbook_fullname'));
						if (isset($data['callsign']['gridsquare'])) {
						...
						$data['error'] = sprintf(__("Lookup not configured or disabled. Please check your %sCallbook Settings%s."), '<a target="_blank" href="' . site_url('options/callbook') . '">', "</a>");
					}
