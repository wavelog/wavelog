...
class Update extends CI_Controller {
	...
	*/
	...
	public function update_wwff_boundaries() {
		set_time_limit(0);
		$lockfilename = '/tmp/.update_wwff_boundaries_running';
		if (file_exists($lockfilename)) {
			$tdiff = time() - filemtime($lockfilename);
			if ($tdiff <= 1800) {
				log_message('debug', 'update_wwff_boundaries: already running, ignoring.');
				echo 'locked - running';
				return;
			}
			unlink($lockfilename);
			log_message('debug', 'update_wwff_boundaries: reclaimed stale lock (older than ' . $tdiff . ' s).');
		}

		touch($lockfilename);
		$this->load->model('Update_model');
		try {
			$result = $this->Update_model->wwff_boundaries();
		} catch (Throwable $e) {
			log_message('error', 'update_wwff_boundaries threw: ' . $e->getMessage());
			$result = 'FAILED: exception - ' . $e->getMessage();
		} finally {
			@unlink($lockfilename);
		}
		...
		if ($this->session->userdata('user_type') == '99') {
			if (substr($result, 0, 4) == 'DONE') {
				$this->session->set_flashdata('success', __("WWFF boundaries update complete. Result: ") . "'" . $result . "'");
			} else {
				$this->session->set_flashdata('error', __("WWFF boundaries update failed. Result: ") . "'" . $result . "'");
			}
			redirect('debug');
		} else {
			echo $result;
