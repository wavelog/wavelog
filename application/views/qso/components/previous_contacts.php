...
function echo_table_col($row, $name) {
	...
	switch($name) {
		...
		case 'Frequency':   
			...
			echo '<td>'; if($row->COL_SAT_NAME != null) { echo '<a href="https://db.satnogs.org/search/?q='.$row->COL_SAT_NAME.'" target="_blank">'.$row->COL_SAT_NAME.'</a></td>'; } else { if($row->COL_FREQ != null) { echo $CI->frequency->unit_conversion($row->COL_FREQ, $qrg_unit); } else { echo strtolower($row->COL_BAND); } } echo '</td>'; break;
