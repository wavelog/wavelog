...
function echo_table_col($row, $name, $adif_propmodes) {
	...
	switch($name) {
		...
		case 'Country': echo '<td>'; if (($row->adif ?? 0) == 0) { echo html_escape($row->name ?? ''); } else echo html_escape(ucwords(strtolower(($row->name ?? '')==null?"- NONE -":($row->name ?? '')))); if (($row->end ?? null) != null) echo ' <span class="badge text-bg-danger">'.__("Deleted DXCC").'</span>'  . '</td>'; break;
