...
<div class="table-responsive">
    <table style="width:100%" class="table contacttable table-striped table-hover">
        <thead>
            <tr class="titles">
            	<!-- Display UTC or Local depending on settings -->
				<th><?= __("Date"); ?><?php if($this->session->userdata('user_show_log_utc') == 1){ echo " (UTC)"; }else{ echo " (Local)"; } ?></th>
                <?php if(($this->config->item('use_auth') && ($this->session->userdata('user_type') >= 2)) || $this->config->item('use_auth') === FALSE || ($this->config->item('show_time'))) { ?>
				<th><?= __("Time"); ?><?php if($this->session->userdata('user_show_log_utc') == 1){ echo " (UTC)"; }else{ echo " (Local)"; } ?></th>
                <?php } ?>
                <th><?= __("Call"); ?></th>
                <?php
        ...
        <?php  $i = 0;
            foreach ($results->result() as $row) {
				...
				<!-- Display different time and tooltip based on settings -->
				<?php if($this->session->userdata('user_show_log_utc') == 1){?>
