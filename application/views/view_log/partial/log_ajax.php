...
<div class="table-responsive">
    <table style="width:100%" class="table contacttable table-striped table-hover">
		...
		<?php if ( strpos($this->session->userdata('user_default_confirmation'),'C') !== false ) { ?>
                    <td class="clublog">
                        <span <?php if ($row->COL_CLUBLOG_QSO_UPLOAD_STATUS == "Y") { echo 'title="'.__("Sent").($row->COL_CLUBLOG_QSO_UPLOAD_DATE != null ? " ".date($custom_date_format, strtotime($row->COL_CLUBLOG_QSO_UPLOAD_DATE)) : '').'" data-bs-toggle="tooltip"'; } elseif ($row->COL_CLUBLOG_QSO_UPLOAD_STATUS == 'M' && $row->COL_CLUBLOG_QSO_UPLOAD_STATUS != NULL) { echo 'title="'.__("Modified")."<br />(".__("last sent")." ".date($custom_date_format, strtotime($row->COL_CLUBLOG_QSO_UPLOAD_DATE)).")".'" data-bs-toggle="tooltip" data-bs-html="true"'; } elseif ($row->COL_CLUBLOG_QSO_UPLOAD_STATUS == 'I') { echo 'title="'.__("Invalid (Ignore)").'" data-bs-toggle="tooltip"'; }?> class="qrz-<?php if ($row->COL_CLUBLOG_QSO_UPLOAD_STATUS == 'Y') { echo 'green'; } elseif ($row->COL_CLUBLOG_QSO_UPLOAD_STATUS == 'M' && $row->COL_CLUBLOG_QSO_UPLOAD_DATE != NULL) { echo 'yellow'; } elseif ($row->COL_CLUBLOG_QSO_UPLOAD_STATUS == 'I') { echo 'grey'; } else { echo 'red'; } ?>">&#9650;</span>
                        <span <?php if ($row->COL_CLUBLOG_QSO_DOWNLOAD_STATUS == "Y") { echo "title=\"".__("Received"); if ($row->COL_CLUBLOG_QSO_DOWNLOAD_DATE != null) { $timestamp = strtotime($row->COL_CLUBLOG_QSO_DOWNLOAD_DATE); echo " ".($timestamp!=''?date($custom_date_format, $timestamp):''); } echo "\" data-bs-toggle=\"tooltip\""; } ?> class="qrz-<?php echo ($row->COL_CLUBLOG_QSO_DOWNLOAD_DATE=='Y')?'green':'red'?>">&#9660;</span>
                    </td>
                <?php } ?>
