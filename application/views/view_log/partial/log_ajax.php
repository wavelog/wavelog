...
<div class="table-responsive">
    <table style="width:100%" class="table contacttable table-striped table-hover">
		...
		<?php if ( strpos($this->session->userdata('user_default_confirmation'),'Z') !== false && ($this->session->userdata('hasQrzKey') != "") ) { ?>
                    <td class="qrz">
                        <span <?php echo ($row->COL_QRZCOM_QSO_UPLOAD_STATUS == "Y") ? 'title="'.__("Sent").(($row->COL_QRZCOM_QSO_UPLOAD_DATE != null) ? " ".date($custom_date_format, strtotime($row->COL_QRZCOM_QSO_UPLOAD_DATE)) : '').'" data-bs-toggle="tooltip"' : (($row->COL_QRZCOM_QSO_UPLOAD_STATUS == 'M' && $row->COL_QRZCOM_QSO_UPLOAD_DATE != NULL) ? 'title="'.__("Modified").'" data-bs-toggle="tooltip"' : ''); ?> class="qrz-<?php echo ($row->COL_QRZCOM_QSO_UPLOAD_STATUS == 'Y') ? 'green' : (($row->COL_QRZCOM_QSO_UPLOAD_STATUS == 'M' && $row->COL_QRZCOM_QSO_UPLOAD_DATE != NULL) ? 'yellow' : 'red'); ?>">&#9650;</span>
