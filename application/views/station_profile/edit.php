...
<div class="container" id="create_station_profile">
	...
	<div class="row">
		...
		<div class="col-md">
			<div class="card">
				<h5 class="card-header"><?php echo $page_title; ?> <?php echo "(" . __("Callsign") . ": "; ?> <?php echo $my_station_profile->station_callsign; ?>)</h5>
				<div class="card-body">
					<div class="mb-3">
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<?php if ($dxcc_list->num_rows() > 0) { ?>
								...
								<option value="" <?php if ($my_station_profile->station_dxcc ?? '' == "") { echo ' selected'; } ?>><?= __("Please select one"); ?></option>
								<?php foreach ($dxcc_list->result() as $dxcc) { ?>
									<?php $isDeleted = $dxcc->end !== NULL; ?>
									<option value="<?php echo $dxcc->adif; ?>"<?php if($my_station_profile->station_dxcc == $dxcc->adif) { echo ' selected'; } ?>>
										<?php
											echo ucwords(strtolower($dxcc->name)) . ' - ' . $dxcc->prefix;
											if ($isDeleted) { echo ' (' . __("Deleted DXCC") . ')'; }
										?>
									</option>
								<?php } ?>
					        </select>
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<select class="form-select" id="stationCQZoneInput" name="station_cq" required>
							...
							for ($i = 1; $i<=40; $i++) {
								echo '<option value='. $i;
								if ($my_station_profile->station_cq == $i) { echo ' selected'; }
								echo '>'. $i .'</option>';
							}
							?>
						</select>
					...
					<div class="mb-3">
						...
						<select class="form-select" id="stationITUZoneInput" name="station_itu" required>
							...
							for ($i = 1; $i<=90; $i++) {
								echo '<option value='. $i;
								if ($my_station_profile->station_itu == $i) { echo ' selected'; }
								echo '>'. $i .'</option>';
							}
							?>
						</select>
						...
						<select class="form-select" name="iota" id="stationIOTAInput" aria-describedby="stationIOTAInputHelp" placeholder="EU-005">
							...
							<?php
								foreach($iota_list as $i){
									...
									if ($my_station_profile->station_iota == $i->tag) { echo ' selected'; }
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<select class="form-select" id="qrzrealtime" name="qrzrealtime">
							<option value="-1" <?php if ($my_station_profile->qrzrealtime == -1) { echo ' selected'; } ?>><?= __("Disabled"); ?></option>
							<option value="1" <?php if ($my_station_profile->qrzrealtime == 1) { echo ' selected'; } ?>><?= __("Realtime"); ?></option>
							<option value="0" <?php if ($my_station_profile->qrzrealtime == 0) { echo ' selected'; } ?>><?= __("Enabled"); ?></option>
						</select>
					</div>
				</div>
			</div>
				...
				<div class="card-body">
					<div class="mb-3">
						...
						<select class="form-select" id="clublogignore" name="clublogignore">
							<option value="1" <?php if ($my_station_profile->clublogignore == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
							<option value="0" <?php if ($my_station_profile->clublogignore == 0) { echo ' selected'; } ?>><?= __("No"); ?></option>
						</select>
					...
					<div class="mb-3" id="clublogrealtimediv">
						...
						<select class="form-select" id="clublogrealtime" name="clublogrealtime">
							<option value="1" <?php if ($my_station_profile->clublogrealtime == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
							<option value="0" <?php if ($my_station_profile->clublogrealtime == 0) { echo ' selected'; } ?>><?= __("No"); ?></option>
						</select>
					</div>
				</div>
			</div>
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<select class="form-select" id="hrdlogrealtime" name="hrdlogrealtime">
							<option value="1" <?php if ($my_station_profile->hrdlogrealtime == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
							<option value="0" <?php if ($my_station_profile->hrdlogrealtime == 0) { echo ' selected'; } ?>><?= __("No"); ?></option>
							<option value="-1" <?php if ($my_station_profile->hrdlogrealtime == -1) { echo ' selected'; } ?>><?= __("Disabled"); ?></option>
						</select>
					</div>
				</div>
			</div>
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<select class="form-select" id="webadifrealtime" name="webadifrealtime">
							<option value="1" <?php if ($my_station_profile->webadifrealtime == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
							<option value="0" <?php if ($my_station_profile->webadifrealtime == 0) { echo ' selected'; } ?>><?= __("No"); ?></option>
						</select>
					</div>
				</div>
			</div>
				...
				<div class="card-body">
					<div class="mb-3">
						...
						<select class="form-select" id="oqrs" name="oqrs">
							<option value="0" <?php if ($my_station_profile->oqrs == 0) { echo ' selected'; } ?>><?= __("No"); ?></option>	
							<option value="1" <?php if ($my_station_profile->oqrs == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
						</select>
					</div>
					<div class="mb-3">
						<label for="oqrs"><?= __("OQRS Email alert"); ?></label>
						<select class="form-select" id="oqrsemail" name="oqrsemail">
							<option value="0" <?php if ($my_station_profile->oqrs_email == 0) { echo ' selected'; } ?>><?= __("No"); ?></option>	
							<option value="1" <?php if ($my_station_profile->oqrs_email == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
						</select>
