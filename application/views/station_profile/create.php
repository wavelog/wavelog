...
<div class="container" id="create_station_profile">
	...
	<div class="row">
		...
		<div class="col-md">
			<div class="card">
				<h5 class="card-header"><?= __("Location"); ?></h5>
				<div class="card-body">
					<!-- DXCC -->
					<div class="mb-3">
						...
						<select class="form-control" id="dxcc_id" name="dxcc" aria-describedby="stationCallsignInputHelp" required>
							...
							<option value="<?php echo $dxcc_item->adif; ?>" <?php if(isset($dxcc) && $dxcc_item->adif == $dxcc) { echo ' selected'; } ?>><?php echo ucwords(strtolower($dxcc_item->name)) . ' - ' . $dxcc_item->prefix; if ($dxcc_item->end != NULL) echo ' ('.__("Deleted DXCC").')';?>
							</option>
							<?php } ?>
						</select>
						<?php } ?>
		...
		<div class="col-md">
			<div class="card">
				<h5 class="card-header"><?= __("Zones"); ?></h5>
				<div class="card-body">
					<!-- CQ Zone -->
					<div class="mb-3">
						...
						<?php
							for ($i = 1; $i<=40; $i++) {
								echo '<option value='.$i;
								if (isset($station_cq) && $station_cq!= '' && $station_cq == $i) { echo ' selected'; }
								echo '>'. $i .'</option>';
							}
						?>
						</select>
					...
					<div class="mb-3">
						...
						<select class="form-select" id="stationITUZoneInput" name="station_itu" required>
							<?php
								for ($i = 1; $i<=90; $i++) {
								echo '<option value='. $i;
								if (isset($station_itu) && $station_itu!= '' && $station_itu == $i) { echo ' selected'; }
								echo '>'. $i .'</option>';
								}
							?>
						</select>
		...
		<div class="col-md">
			<div class="card">
				...
				<div class="card-body">
                  ...
                  <span class="badge text-bg-warning"><?= __("Warning"); ?></span> <small id="stationGridInputHelp" class="form-text text-muted"><b><?= __("This is needed for all features based on current location, e.g. maps, distance statistics and more. Make sure to fill in the gridsquare if you want to use those features"); ?></b></small>
					</div>
				</div>
			</div>
		</div>
						...
						<select class="form-select" name="iota" id="stationIOTAInput" aria-describedby="stationIOTAInputHelp" placeholder="EU-005">
							...
							<?php
								foreach($iota_list as $i){
									...
									if(isset($iota) && $i->tag == $iota) { echo ' selected'; }
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<select class="form-select" id="qrzrealtime" name="qrzrealtime">
							<option value="-1" <?php if (!isset($qrzrealtime) || ($qrzrealtime != 1 && $qrzrealtime != 0)) { echo ' selected'; } ?>><?= __("Disabled"); ?></option>
							<option value="1" <?php if (isset($qrzrealtime) && $qrzrealtime == 1) { echo ' selected'; } ?>><?= __("Realtime"); ?></option>
							<option value="0" <?php if (isset($qrzrealtime) && $qrzrealtime == 0) { echo ' selected'; } ?>><?= __("Enabled"); ?></option>
						</select>
					</div>
				</div>
			</div>
	...
	<div class="row">
		...
		<div class="col-md">
			<div class="card">
				<h5 class="card-header"><?= __("Clublog"); ?></h5>
				<div class="card-body">
					<div class="mb-3">
						<label for="clublogignore"><?= __("Ignore Clublog Upload"); ?></label>
						<select class="form-select" id="clublogignore" name="clublogignore">
							<option value="1" <?php if (isset($clublogignore) && $clublogignore == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
							<option value="0" <?php if (!isset($clublogignore) || $clublogignore != 1) { echo ' selected'; } ?>><?= __("No"); ?></option>
						</select>
					...
					<div class="mb-3" id="clublogrealtimediv">
						...
						<select class="form-select" id="clublogrealtime" name="clublogrealtime">
							<option value="1" <?php if (isset($clublogrealtime) && $clublogrealtime == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
							<option value="0" <?php if (!isset($clublogrealtime) || $clublogrealtime != 1) { echo ' selected'; } ?>><?= __("No"); ?></option>
						</select>
					</div>
				</div>
			</div>
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<select class="form-select" id="hrdlogrealtime" name="hrdlogrealtime">
							<option value="1" <?php if (isset($hrdlogrealtime) && $hrdlogrealtime == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
							<option value="0" <?php if (isset($hrdlogrealtime) && $hrdlogrealtime == 0) { echo ' selected'; } ?>><?= __("No"); ?></option>
							<option value="-1" <?php if (!isset($hrdlogrealtime) || ($hrdlogrealtime != 1 && $hrdlogrealtime != 0)) { echo ' selected'; } ?>><?= __("Disabled"); ?></option>
						</select>
					</div>
				</div>
			</div>
				...
				<div class="card-body">
					...
					<div class="mb-3">
						...
						<select class="form-select" id="webadifrealtime" name="webadifrealtime">
							<option value="1" <?php if (isset($webadifrealtime) && $webadifrealtime == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
							<option value="0" <?php if (!isset($webadifrealtime) || $webadifrealtime != 1) { echo ' selected'; } ?>><?= __("No"); ?></option>
						</select>
					</div>
				</div>
			</div>
				...
				<div class="card-body">
					<div class="mb-3">
						...
						<select class="form-select" id="oqrs" name="oqrs">
							<option value="0" <?php if (!isset($oqrs) || $oqrs != 1) { echo ' selected'; } ?>><?= __("No"); ?></option>
							<option value="1" <?php if (isset($oqrs) && $oqrs == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
						</select>
					</div>
					<div class="mb-3">
						<label for="oqrs"><?= __("OQRS Email alert"); ?></label>
						<select class="form-select" id="oqrsemail" name="oqrsemail">
							<option value="0" <?php if (!isset($oqrsemail) || $oqrsemail != 1) { echo ' selected'; } ?>><?= __("No"); ?></option>
							<option value="1" <?php if (isset($oqrsemail) && $oqrsemail == 1) { echo ' selected'; } ?>><?= __("Yes"); ?></option>
						</select>
...
<?php } ?>
	...
	<button type="submit" class="btn btn-primary" style="margin-bottom: 30px;"><i class="fas fa-plus-square"></i> <?= __("Create"); ?> <?= __("Station Location"); ?></button>
