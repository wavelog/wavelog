...
<div class="container px-3 px-lg-4 mt-3 mb-3">
    ...
    <div id="awardInfoButton" class="mb-3">
        ...
        var lang_award_info_ln1 = "<?= __("JCG - Japan Century Gun/County Award"); ?>";
        var lang_award_info_ln2 = "<?= __("Companion award to JCG covering Japan's gun (county) and equivalent rural entities rather than cities. Claimed for having contacted (heard) and received a QSL card from an amateur station located in each of a number of different guns of Japan."); ?>";
        var lang_award_info_ln3 = "<?= __("A list of QSL cards should be arranged in order of JCG reference number, however names of gun may be omitted."); ?>";
        ...
        var lang_award_info_ln5 = "<?= __("Fields taken for this Award: DXCC (Japan) and County (Must contain a valid JCG reference!)"); ?>";
        </script>
    ...
    <div class="card">
        <div class="card-header">
            <ul class="nav nav-tabs card-header-tabs" id="jcg-tabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link active" id="jcg-results-tab" data-bs-toggle="tab" href="#jcg-results" role="tab" aria-controls="jcg-results" aria-selected="true"><i class="fas fa-table"></i> <?= __("Results"); ?></a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="jcg-map-tab" onclick="load_jcg_map();" data-bs-toggle="tab" href="#jcg-map-panel" role="tab" aria-controls="jcg-map-panel" aria-selected="false"><i class="fas fa-map"></i> <?= __("Map"); ?></a>
                </li>
            </ul>
        </div>
        <div class="card-body">
            <form class="form" action="<?php echo site_url('awards/jcg'); ?>" method="post" enctype="multipart/form-data">
                <div class="mb-4 text-center">
                    <div class="dropdown" data-bs-auto-close="outside">
                        ...
                        <button type="button" onclick="load_jcg_map();" class="btn btn-info btn-sm"><i class="fas fa-globe-asia"></i> <?= __("Show JCG Map"); ?></button>
                        <button id="button3id" type="button" onclick="export_qsos();" name="button3id" class="btn btn-sm btn-info"<?php echo (($jcg_summary['confirmed'] ?? 0) == 0) ? ' disabled' : ''; ?>><?= __("Export confirmed QSOs"); ?></button>
            ...
            <div class="tab-content" id="jcg-tab-content">
                <div class="tab-pane fade show active" id="jcg-results" role="tabpanel" aria-labelledby="jcg-results-tab">
                    <div class="border rounded px-3 py-2 mb-3">
                        <div class="d-flex flex-column flex-xl-row justify-content-between align-items-start align-items-xl-center gap-3">
                            <div class="d-flex flex-wrap align-items-center gap-3 gap-xl-4">
                                <div class="d-inline-flex align-items-baseline gap-2">
                                    ...
                                    <span class="fs-5 fw-bold text-success"><?php echo $jcg_summary['confirmed']; ?></span>
                                    <span class="small"><?php echo number_format($jcg_summary['confirmed_percent'], 1); ?>%</span>
                                </div>
                                <div class="d-inline-flex align-items-baseline gap-2">
                                    <span class="small"><?= __("Worked"); ?></span>
                                    <span class="fs-5 fw-bold text-danger"><?php echo $jcg_summary['worked']; ?></span>
                                    <span class="small"><?php echo number_format($jcg_summary['worked_percent'], 1); ?>%</span>
                                </div>
                                <div class="d-inline-flex align-items-baseline gap-2">
                                    <span class="small"><?= __("Total"); ?></span>
                                    <span class="fs-5 fw-bold"><?php echo $jcg_summary['total']; ?></span>
                                </div>
                                <?php if (($postdata['includedeleted'] ?? null) == 1) { ?>
                                    <div class="d-inline-flex align-items-baseline gap-2">
                                        <span class="small"><?= __("Deleted"); ?></span>
                                        <span class="fs-5 fw-bold"><?php echo $jcg_summary['deleted']; ?></span>
                                    </div>
                                <?php } ?>
                            </div>
                        ...
                        <div class="progress award-grid-progress mt-2" role="progressbar" aria-label="<?= __("JCG progress"); ?>" aria-valuenow="<?php echo (int) round($jcg_summary['worked_percent']); ?>" aria-valuemin="0" aria-valuemax="100">
                            <div class="progress-bar bg-success" style="width: <?php echo $jcg_summary['confirmed_percent']; ?>%"></div>
                            <?php if (($jcg_summary['worked_only_percent'] ?? 0) > 0) { ?>
                                <div class="progress-bar bg-danger" style="width: <?php echo $jcg_summary['worked_only_percent']; ?>%"></div>
                            <?php } ?>
                        </div>
                    </div>
                    ...
                    <?php if (!$has_active_slots) { ?>
                        <div class="alert alert-danger" role="alert">
                            <?= __("No worked or confirmed JCG slots match the current filters."); ?>
                        </div>
                    <?php } ?>

                    <div class="border-top">
                        <?php foreach ($jcg_groups as $group) { ?>
                ...
                <div class="tab-pane fade" id="jcg-map-panel" role="tabpanel" aria-labelledby="jcg-map-tab">
                    <div id="jcgmap" class="map-leaflet"></div>
                </div>
            </div>
        </div>
    </div>
