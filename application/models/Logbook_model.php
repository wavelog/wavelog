	...
	public function check_missing_grid_id($all) {
		...
		if ($r->num_rows() > 0) {
			foreach ($r->result_array() as $row) {
				...
				if ($this->optionslib->get_option('callbook_provider') == "hamqth" && $this->optionslib->get_option('callbook_username') != null && $this->optionslib->get_callbook_password() != null) {
		...
		try {
			if ($this->optionslib->get_option('callbook_provider') == "qrz" && $this->optionslib->get_option('callbook_username') != null && $this->optionslib->get_callbook_password() != null) {
				...
				if ($callbook['error'] ?? '' == 'Invalid session key') {
					$this->qrz->set_session($this->optionslib->get_option('callbook_username'), $this->optionslib->get_callbook_password());
					$callbook = $this->qrz->search($callsign, $this->session->userdata('qrz_session_key'), $use_fullname);
				}
